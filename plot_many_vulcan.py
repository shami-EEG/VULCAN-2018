...
vul_data = 'output/new-H2O-HCN-NO2-HD189.vul'
vul_data2 = 'output/no-uv-v2718-HD189.vul'
vul_data3 = 'output/new-H2O-HCN-NO2-HD189.vul'
...
for sp in plot_spec:
    ...
    plt.plot(data['variable']['ymix'][:,vulcan_spec.index(sp)], data['atm']['pco']/1.e6, color=colors[color_index], label=sp_lab, alpha=0.9)
    ...
    if sp in data2['variable']['species']:
        plt.plot(data2['variable']['ymix'][:,vulcan_spec2.index(sp)], data2['atm']['pco']/1.e6, color=colors[color_index], ls='--',lw=1.2, alpha=0.9)
   
    # if sp in data3['variable']['species']:
    #     plt.plot(data3['variable']['ymix'][:,vulcan_spec2.index(sp)], data3['atm']['pco']/1.e6, color=colors[color_index], ls='-.',lw=1.8, alpha=0.8)
