...
bool FastChem<double_type>::nelderMeadSimplexSolve(Element<double_type>& species, const double_type initial_solution, const double h_density, const unsigned int grid_index)
{
  ...
  for (unsigned int iter_step = 0; iter_step < nb_iterations; ++iter_step)
  {
    ...
    if (vf_epsilon_minus < 0 && vf_epsilon_plus > 0)
    {
      converged = true;
    ...
    if (fxr < vf[x1])
    {
      ...
      xe = (1.0 + rho*chi) * xg - rho*chi*x[xnp1];  //expansion
    ...
    else
    {
      ...
      else
      {
        ...
        if (fxr < vf[xnp1])
        {
          ...
          xc = (1.0 + psi*rho) * xg - psi*rho*x[xnp1];  //outward contraction
        ...
        else
        {
          ...
          xc = (1.0 - psi) * xg + psi*rho*x[xnp1];  //inward contraction
        ...
        if (perform_shrink)
        {
          ...
          {
            if (i != x1)
              x[i] = x[x1] + sigma*(x[i] - x[x1]);  //total contraction
          }
