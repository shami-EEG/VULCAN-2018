                                                ...
                                                unsigned int& nb_pressure_iterations_out, unsigned int& nb_chemistry_iterations_out)
{
  ...
  double_type h_density = 1./(2. * std::pow(1 + 2.*elements[He_].abundance,2) * std::exp(molecules[H2_].mass_action_constant[grid_index]))
                          * ( (1. + elements[He_].abundance)
                               + 4*(1 + 2*elements[He_].abundance) * std::exp(molecules[H2_].mass_action_constant[grid_index]) * total_density
                               - std::sqrt(std::pow(1. + elements[He_].abundance,2) + 4.*(1. + 2.*elements[He_].abundance)*std::exp(molecules[H2_].mass_action_constant[grid_index])*total_density));
